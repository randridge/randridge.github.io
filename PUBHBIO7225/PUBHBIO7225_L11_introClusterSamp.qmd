...
## Cluster Sampling: A Classic Example {.pause}
...
## So Why Would We Cluster Sample? {.pause}
...
## Vocabulary of Cluster Sampling {.pause}
...
+----------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------+
| ![](figures/str_vs_clus_pop.png){fig-alt="set of rectangles representing strata, each divided into squares representing observation units." width="4in"} | ![](figures/str_vs_clus_pop.png){fig-alt="set of rectangles representing strata, each divided into squares representing observation units." width="4in"} |
+----------------------------------------------------------------------------------------------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------------------------------------------+
...
-   We have $\bar{t}$, how can we get $\hat{t}_{clus}$?
    ...
    -   [Truth:]{.alert} $N \times \bar{t}_U = t_U$   →   [Estimate:]{.alert2} $N \times \bar{t}= \hat{t}_{clus}$
...
## Sampling Weights in One-Stage Cluster Samples {.pause}
...
::: {.column width="30%"}
![](./figures/cluster_between_small){width="\\textwidth" fig-alt="Scatter plot of five clusters. Data points within each cluster are spread widely along the y-axis. The red squares, marking the cluster means, are all located at very similar y-values, indicating small separation between cluster means."}
:::
:::::
...
::: {.column width="30%"}
![](./figures/cluster_between_big){width="\\textwidth" fig-alt="Scatter plot of five clusters. Data points within each cluster are tightly grouped along the y-axis. The red squares, marking the cluster means, are located at distinct, well-separated y-values, indicating large separation between cluster means."}
:::
:::::
:::::::::
...
-   Design effect (DEFF) for the estimated overall mean from 1-stage cluster sample[^1]: $$\begin{aligned}
...
[^1]: Full derivation on Carmen if you're interested (OPTIONAL)

## Example Calculation of DEFF (from Activity) {.pause}
