...
$$\begin{flalign}
\text{Expectation } = E(\bar{y}) &= \sum_k k P(\bar{y}=k) & \\
...
-   Though, notice that [no sample actually produces the true mean!]{.alert}

$$\begin{flalign}
\text{Variance }=V(\bar{y}) &= \sum_k (k-E(\bar{y}))^2 P(\bar{y}=k) & \\
& = (15-58)^2 \times 0.1 + (30-58)^2 \times 0.1 + \cdots + (105-58)^2 \times 0.1 = \mathbf{921}
\end{flalign}$$

-   Note that we **did not make any assumption about the distribution of the** $\mathbf{y}$ **values** -- the "randomness" is from the sampling scheme, from the fact that a different sample will produce a different value of $\bar{y}$.
...
-   Using this [**auxiliary data**]{.alert2} -- the number of doctors at each clinic -- can help create a more efficient sampling scheme
...
> [**Mean Squared Error (MSE):**]{.alert2} Variance + Bias-squared &nbsp; $$MSE(\hat{\theta}) = E[(\hat{\theta}-\theta)^2] = V(\hat{\theta}) + [\text{Bias}(\hat{\theta})]^2$$
