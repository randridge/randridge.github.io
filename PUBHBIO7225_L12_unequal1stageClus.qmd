...
4.  Estimated SE of the mean: $\widehat{SE}(\hat{\bar{y}}_{clus}) = \sqrt{\text{`r round(SE(tot)^2/M0^2,1)`}} = \text{`r round(sqrt(SE(tot)^2/M0^2),1)`}$
    ...
    -   That is a pretty big standard error!
    -   A 95% CI would be roughly `r round(coef(tot)/M0,1)` $\pm$ `r round(sqrt(SE(tot)^2/M0^2),1)` = (`r round(coef(tot)/M0 - 2*sqrt(SE(tot)^2/M0^2),1)`, `r round(coef(tot)/M0 + 2*sqrt(SE(tot)^2/M0^2),1)`)
...
-   Ratio estimate of the mean is [larger]{.alert} than the unbiased estimate [for this sample]{.alert} -- why?
...
-   To get the estimated variance, plug in $\hat{t}_{clus}$ and $\widehat{M}_0$ (in place of $t_U$ and $M_0$) in the variance expression above
...
-   If the cluster sizes are all the same size:
    ...
    -   $V(\widehat{M}_0) = 0$ because every sample of $n$ PSUs contains the same number of SSUs
...
-   Compare to the unbiased estimator (previously calculated):
    ...
    -   $\hat{\bar{y}}_{clus}= \text{`r round(coef(tot)/M0,1)`}$

    -   $\widehat{SE}(\hat{\bar{y}}_{clus}) = \text{`r round(sqrt(SE(tot)^2/M0^2),1)`}$
